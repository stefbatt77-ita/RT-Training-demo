 ...
 * For full access, request Beta invitation at: rtsymulationtrainingfeedback@gmail.com
 * 
 * Version: 1.0.0-demo
...
// ═══════════════════════════════════════════════════════════════════════════
const IS_DEMO = true;
const DEMO_TIME_LIMIT = 15 * 60; // 15 minutes in seconds
const DEMO_FEATURES = {
  allowedModes: ['teaching'], // Only teaching mode
  allowedMaterials: ['aluminum'], // Only aluminum
  allowedSpecimenTypes: ['plate'], // Only flat plate, no welds
  allowedDefectTypes: ['crack', 'porosity', 'inclusion'], // Only 3 basic defects
  allowedIQI: ['iso'], // Only ISO wire IQI
  allowExam: false,
  allowCertificates: false,
  allowMultiImageExam: false,
  maxDefectsPerImage: 3
};
const BETA_REQUEST_URL = "mailto:rtsymulationtrainingfeedback@gmail.com?subject=Richiesta%20Accesso%20Beta%20RT%20Training&body=Buongiorno%2C%0A%0ASono%20interessato%20ad%20accedere%20alla%20versione%20Beta%20completa%20di%20RT%20Training.%0A%0ANome%3A%20%0AAzienda%3A%20%0ARuolo%3A%20%0A%0AGrazie";
...
const translations = {
  en: {
    ...
    // Demo version translations
    demoVersion: "DEMO VERSION",
    demoTimeRemaining: "Demo time remaining",
    demoExpired: "Demo Time Expired",
    demoExpiredMessage: "Your 15-minute demo session has ended. Request access to the full Beta version to continue training!",
    requestBetaAccess: "Request Beta Access",
    availableInFullVersion: "Available in Full Version",
    featureLockedTitle: "Feature Locked",
    featureLockedMessage: "This feature is only available in the full Beta version.",
    demoLimitations: "Demo Limitations",
    demoOnlyTeaching: "Only Teaching mode available",
    demoOnlyAluminum: "Only Aluminum material",
    demoOnlyPlate: "Only Flat Plate specimen",
    demoOnly3Defects: "Only 3 basic defect types",
    demoNoExam: "Exam mode not available",
    demoNoCertificates: "Certificates not available",
    upgradeNow: "Upgrade Now",
    tryFullVersion: "Try the Full Version",
  ...
  it: {
    ...
    // Demo version translations
    demoVersion: "VERSIONE DEMO",
    demoTimeRemaining: "Tempo demo rimanente",
    demoExpired: "Tempo Demo Scaduto",
    demoExpiredMessage: "La tua sessione demo di 15 minuti è terminata. Richiedi l'accesso alla versione Beta completa per continuare l'addestramento!",
    requestBetaAccess: "Richiedi Accesso Beta",
    availableInFullVersion: "Disponibile nella Versione Completa",
    featureLockedTitle: "Funzione Bloccata",
    featureLockedMessage: "Questa funzione è disponibile solo nella versione Beta completa.",
    demoLimitations: "Limitazioni Demo",
    demoOnlyTeaching: "Solo modalità Insegnamento",
    demoOnlyAluminum: "Solo materiale Alluminio",
    demoOnlyPlate: "Solo campione Piastra Piana",
    demoOnly3Defects: "Solo 3 tipi di difetti base",
    demoNoExam: "Modalità Esame non disponibile",
    demoNoCertificates: "Certificati non disponibili",
    upgradeNow: "Passa alla Versione Completa",
    tryFullVersion: "Prova la Versione Completa",
...
// ═══════════════════════════════════════════════════════════════════════════
// DEMO VERSION: Simplified AuthProvider with auto-login
// ═══════════════════════════════════════════════════════════════════════════
const AuthProvider = ({ children }) => {
  ...
  // No login/logout needed for demo - always logged in
  const login = async () => ({ success: true });
  const logout = async () => {};
  const register = async () => ({ success: true });
  
  // Stub functions for compatibility
  const purchaseSingleSession = async () => ({ success: false, error: 'Non disponibile in demo' });
  const activateSingleSession = async () => ({ success: false, error: 'Non disponibile in demo' });
  const updateUser = async () => ({ success: true });
  const redeemSessionCode = async () => ({ success: false, error: 'Non disponibile in demo' });
  const getSessionRemainingTime = () => null;
  const isSessionActive = () => false;
  const expireSingleSession = async () => {};
  const clearSingleSession = async () => {};
  const startFreeDemo = async () => ({ success: false, error: 'Già in modalità demo' });
...
// Locked Feature Overlay - shows on locked features
const LockedFeatureOverlay = ({ t, featureName }) => (
  <div className="absolute inset-0 bg-gray-900/90 flex flex-col items-center justify-center z-40 backdrop-blur-sm">
    <div className="bg-gradient-to-br from-yellow-600 to-orange-600 p-1 rounded-xl">
      <div className="bg-gray-900 rounded-lg p-6 text-center max-w-sm">
        <div className="w-16 h-16 mx-auto mb-4 bg-yellow-500/20 rounded-full flex items-center justify-center">
          <Shield className="w-8 h-8 text-yellow-500" />
        </div>
        <h3 className="text-xl font-bold text-white mb-2">{t.availableInFullVersion}</h3>
        <p className="text-gray-400 mb-4 text-sm">{featureName}</p>
        <a 
          href={BETA_REQUEST_URL}
          className="inline-flex items-center gap-2 bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400 text-black font-bold py-2 px-6 rounded-lg transition"
        >
          <Star className="w-4 h-4" />
          {t.requestBetaAccess}
        </a>
...
// Demo Expired Modal
const DemoExpiredModal = ({ t }) => (
  <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[100]">
    <div className="bg-gradient-to-br from-red-600 to-orange-600 p-1 rounded-2xl animate-pulse">
      <div className="bg-gray-900 rounded-xl p-8 text-center max-w-md">
        <div className="w-20 h-20 mx-auto mb-6 bg-red-500/20 rounded-full flex items-center justify-center">
          <Timer className="w-10 h-10 text-red-500" />
        </div>
        <h2 className="text-2xl font-bold text-white mb-3">{t.demoExpired}</h2>
        <p className="text-gray-300 mb-6">{t.demoExpiredMessage}</p>
        
        <div className="space-y-3">
          <a 
            href={BETA_REQUEST_URL}
            className="block w-full bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-white font-bold py-3 px-6 rounded-lg transition text-lg"
          >
            🚀 {t.requestBetaAccess}
          </a>
          <button 
            onClick={() => window.location.reload()}
            className="block w-full bg-gray-700 hover:bg-gray-600 text-white font-medium py-2 px-6 rounded-lg transition text-sm"
          >
            🔄 Ricomincia Demo
          </button>
        </div>
...
// Demo Watermark - permanent overlay
const DemoWatermark = ({ t }) => (
  <div className="fixed top-0 left-0 right-0 bg-gradient-to-r from-yellow-600 via-orange-500 to-yellow-600 text-black py-1 px-4 flex items-center justify-center gap-4 z-50 text-sm font-bold">
    <span className="animate-pulse">⚠️</span>
    <span>{t.demoVersion} - {t.demoLimitations}: {t.demoOnlyTeaching}</span>
    <a 
      href={BETA_REQUEST_URL}
      className="bg-black text-yellow-400 px-3 py-0.5 rounded-full text-xs hover:bg-gray-800 transition"
    >
      {t.requestBetaAccess} →
    </a>
  </div>
...
// Demo Timer Display
const DemoTimerDisplay = ({ timeRemaining, t }) => {
  const minutes = Math.floor(timeRemaining / 60);
  const seconds = timeRemaining % 60;
  const isLow = timeRemaining < 180; // Less than 3 minutes
  ...
  return (
    <div className={`fixed bottom-4 right-4 z-50 px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 ${
      isCritical ? 'bg-red-600 animate-pulse' : isLow ? 'bg-orange-600' : 'bg-gray-800'
    }`}>
      <Timer className="w-4 h-4" />
      <span className="font-mono font-bold">
        {String(minutes).padStart(2, '0')}:{String(seconds).padStart(2, '0')}
      </span>
      <span className="text-xs opacity-75">{t.demoTimeRemaining}</span>
    </div>
  );
};

// Demo Limitations Info Panel
const DemoLimitationsPanel = ({ t, onRequestBeta }) => (
  <div className="bg-gradient-to-br from-yellow-900/30 to-orange-900/30 border border-yellow-600/50 rounded-lg p-4 mb-4">
    ...
    <ul className="text-xs text-yellow-200/80 space-y-1 mb-3">
      <li>• {t.demoOnlyTeaching}</li>
      <li>• {t.demoOnlyAluminum}</li>
      <li>• {t.demoOnlyPlate}</li>
      <li>• {t.demoOnly3Defects}</li>
      <li>• {t.demoNoExam}</li>
    </ul>
    <a 
      href={BETA_REQUEST_URL}
      className="block w-full text-center bg-yellow-600 hover:bg-yellow-500 text-black font-bold py-2 px-3 rounded text-xs transition"
    >
      {t.tryFullVersion} →
    </a>
  </div>
...
const XRaySimulator = ({ onExamComplete }) => {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // DEMO VERSION: Timer and limitations
  // ═══════════════════════════════════════════════════════════════════════════
  const [demoTimeRemaining, setDemoTimeRemaining] = useState(DEMO_TIME_LIMIT);
  ...
  // Function to show locked feature modal
  const showLockedFeature = (featureName) => {
    setLockedFeatureName(featureName);
    setShowLockedModal(true);
  };
  
  // Mode: 'teaching' only in DEMO (learning and exam locked)
  const [mode, setMode] = useState('teaching');
  // DEMO: Force aluminum only
  ...
  // Multi-image exam state (5 images per exam) - LOCKED IN DEMO
  ...
  const [showMissedDefects, setShowMissedDefects] = useState(false);
...
═══════════════════════════════════════════════════════
  ...
  return (
    <div className="flex-1 flex flex-col bg-gray-950 h-full overflow-hidden">
      {/* ═══════════════════════════════════════════════════════════════════════════ */}
      {/* DEMO VERSION OVERLAYS */}
      {/* ═══════════════════════════════════════════════════════════════════════════ */}
      
      {/* Demo Watermark - always visible */}
      {IS_DEMO && <DemoWatermark t={t} />}
      
      {/* Demo Timer - bottom right */}
      {IS_DEMO && !demoExpired && <DemoTimerDisplay timeRemaining={demoTimeRemaining} t={t} />}
      
      {/* Demo Expired Modal - blocks everything when time runs out */}
      {IS_DEMO && demoExpired && <DemoExpiredModal t={t} />}
      
      {/* Locked Feature Modal */}
      {showLockedModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60]" onClick={() => setShowLockedModal(false)}>
          <div className="bg-gradient-to-br from-yellow-600 to-orange-600 p-1 rounded-xl" onClick={e => e.stopPropagation()}>
            <div className="bg-gray-900 rounded-lg p-6 text-center max-w-sm">
              <div className="w-16 h-16 mx-auto mb-4 bg-yellow-500/20 rounded-full flex items-center justify-center">
                <Shield className="w-8 h-8 text-yellow-500" />
              </div>
              <h3 className="text-xl font-bold text-white mb-2">{t.featureLockedTitle}</h3>
              <p className="text-gray-400 mb-2">{lockedFeatureName}</p>
              <p className="text-gray-500 text-sm mb-4">{t.featureLockedMessage}</p>
              <a 
                href={BETA_REQUEST_URL}
                className="inline-flex items-center gap-2 bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400 text-black font-bold py-2 px-6 rounded-lg transition"
              >
              ...
              <button 
                ...
                className="block w-full mt-3 text-gray-500 hover:text-gray-300 text-sm"
              >
                Chiudi
              </button>
            </div>
      ...
      {/* Add top padding for watermark */}
      <div className={`bg-gray-900 border-b border-gray-700 px-4 py-2 flex items-center gap-4 flex-wrap ${IS_DEMO ? 'mt-8' : ''}`}>
        <div className="flex gap-1">
          {/* Teaching Mode - Always available */}
          <button 
          ...
          {/* Learning Mode - LOCKED IN DEMO */}
          <button 
            onClick={() => IS_DEMO ? showLockedFeature(t.learning + ' - ' + t.availableInFullVersion) : handleModeChange('learning')} 
            className={`flex items-center gap-2 px-3 py-2 text-sm font-semibold transition relative ${
              IS_DEMO 
                ? 'bg-gray-800 text-gray-500 cursor-not-allowed' 
                : mode === 'learning' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
            }`}
            title={IS_DEMO ? t.availableInFullVersion : t.learningDesc}
          >
            <BookOpen className="w-4 h-4" />
            <span className="hidden sm:inline">{t.learning}</span>
            {IS_DEMO && <Shield className="w-3 h-3 absolute -top-1 -right-1 text-yellow-500" />}
          </button>
          
          {/* Exam Mode - LOCKED IN DEMO */}
          <button 
            onClick={() => IS_DEMO ? showLockedFeature(t.exam + ' - ' + t.availableInFullVersion) : handleModeChange('exam')} 
            className={`flex items-center gap-2 px-3 py-2 rounded-r text-sm font-semibold transition relative ${
              IS_DEMO 
                ? 'bg-gray-800 text-gray-500 cursor-not-allowed border border-gray-600' 
                : mode === 'exam' ? 'bg-red-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
            }`}
            title={IS_DEMO ? t.availableInFullVersion : t.examDesc}
          >
        ...
        {/* Training Time Display - only in teaching mode for demo */}
        {mode === 'teaching' && (
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        <div className="w-64 bg-gray-900 border-r border-gray-700 overflow-y-auto p-4 space-y-4 flex-shrink-0">
          ...
          {/* ═══════════════════════════════════════════════════════════════════════════ */}
          {/* DEMO LIMITATIONS PANEL */}
          {/* ═══════════════════════════════════════════════════════════════════════════ */}
          {IS_DEMO && <DemoLimitationsPanel t={t} />}
          ...
          {/* Specimen Type Selector - WELD LOCKED IN DEMO */}
          <div className="border-t border-gray-700 pt-4">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => IS_DEMO ? showLockedFeature(t.weldedJoint + ' - ' + t.availableInFullVersion) : setSpecimenType('weld')}
                disabled={examStarted}
                className={`flex-1 px-3 py-2 rounded text-sm font-medium transition relative ${
                  IS_DEMO 
                    ? 'bg-gray-800 text-gray-500 cursor-not-allowed border border-dashed border-yellow-600' 
                    : specimenType === 'weld' 
            ...
            {IS_DEMO && (
              <p className="text-xs text-yellow-500 mt-2 flex items-center gap-1">
                ...
                {t.availableInFullVersion}
              </p>
            )}
            {!IS_DEMO && specimenType === 'weld' && (
          ...
          {/* Material Selector - DEMO: Only Aluminum */}
          <div className="border-t border-gray-700 pt-4">
            <label className="block text-sm font-medium mb-2">{t.material}</label>
            {IS_DEMO ? (
              <>
                <div className="w-full bg-gray-800 rounded px-3 py-2 text-white border border-gray-700">
                  {t.aluminium} (Al)
                </div>
                <p className="text-xs text-yellow-500 mt-1 flex items-center gap-1">
                  <Shield className="w-3 h-3" />
                  Titanio, Inconel: {t.availableInFullVersion}
                </p>
              </>
            ) : (
              <>
